...
DROP POLICY IF EXISTS "pledge_update_policy" ON public.pledges;
DROP POLICY IF EXISTS "pledge_delete_policy" ON public.pledges;
DROP POLICY IF EXISTS "Admins can manage all pledges" ON public.pledges;

-- Step 5: Create new secure RLS policies using only user_id (no wallet address authentication)
CREATE POLICY "secure_pledges_select"
ON public.pledges
FOR SELECT
TO authenticated
USING (
...
CREATE POLICY "secure_pledges_insert"
ON public.pledges
FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "secure_pledges_update"
ON public.pledges
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY "secure_pledges_delete"
ON public.pledges
FOR DELETE
TO authenticated
USING (
...
-- Step 7: Add constraint to ensure user_id references valid users
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
        WHERE constraint_name = 'fk_pledges_user_id'
    ) THEN
        ALTER TABLE public.pledges 
