...
-- Create specific, secure policies for token_balances

-- Allow users to view their own token balances and admins to view all
CREATE POLICY "Users can view own balances, admins view all" 
...
CREATE POLICY "Service role and admins can insert balances" 
ON public.token_balances 
FOR INSERT 
WITH CHECK (public.is_service_role_or_admin());

-- Only service role (edge functions) and admins can update token balances
CREATE POLICY "Service role and admins can update balances" 
ON public.token_balances 
FOR UPDATE 
